...
export function createChart(spec) {
  ...
  function render() {
    ...
    // gridlines + ticks
    const xt = niceTicks(view.xMin, view.xMax, 5, xLog);
    const xFmt = spec.xFmt || (xLog ? fmtNum : fmtForStep(xt.step));
    for (const tv of xt) {
      const px = xScale(tv);
      if (px < ML - 0.5 || px > ML + innerW + 0.5) continue;
      svg.appendChild(line(px, MT, px, MT + innerH, '#cfcdc2', 0.5));
      svg.appendChild(text(px, MT + innerH + 17, xFmt(tv), 'middle', 'plot-tick'));
    }
    const yt = niceTicks(view.yMin, view.yMax, 8, false);
    const yFmt = spec.yFmt || fmtForStep(yt.step);
    for (const tv of yt) {
      const py = yScale(tv);
      if (py < MT - 0.5 || py > MT + innerH + 0.5) continue;
      svg.appendChild(line(ML, py, ML + innerW, py, '#cfcdc2', 0.5));
      svg.appendChild(text(ML - 8, py + 4, yFmt(tv), 'end', 'plot-tick'));
    }
...
function text(x, y, str, anchor, cls) {
  ...
  if (cls === 'plot-tick') {
    // tick numbers sit in the dark MARGIN (below/left of the light panel) -> light text
    t.setAttribute('fill', '#e3e6ee');
    t.setAttribute('font-size', '12.5');
    t.setAttribute('font-weight', '600');
    t.setAttribute('font-family', 'ui-monospace, monospace');
  } else if (cls === 'plot-axis') {
    // axis titles also sit on the dark page -> light text
    t.setAttribute('fill', '#f0f2f7');
    t.setAttribute('font-size', '13.5');
    t.setAttribute('font-weight', '600');
  } else if (cls === 'plot-legend') {
    // legend sits INSIDE the light panel -> keep curve colors (dark enough)
    t.setAttribute('font-size', '12.5');
...
// Build a formatter that shows enough decimals to distinguish ticks spaced
// `step` apart. Used for axes where the data range is narrow (e.g. a 12 V
// output with millivolt ripple) so labels don't all collapse to "12.0".
function fmtForStep(step) {
  const s = Math.abs(step);
  if (s === 0 || !isFinite(s)) return fmtNum;
  if (s >= 1e4) return v => v.toExponential(1);
  // decimals needed so the step is visible: e.g. step 0.03 -> 2 decimals,
  // step 0.003 -> 3 decimals. One extra place beyond the step's magnitude.
  let decimals = Math.max(0, Math.ceil(-Math.log10(s)) + 1);
  if (decimals > 6) return v => v.toExponential(2);
...
function niceTicks(min, max, count, isLog) {
  ...
  if (min === max) { const t = [min]; t.step = 0; return t; }
